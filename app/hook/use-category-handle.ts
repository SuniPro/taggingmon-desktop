...
export type CategoryInput = { name: string; is_default: boolean };

export const AutoCategoriesAndInfoGenerator = (
	fsoList: FsoInfo[],
): { name: string; is_default: boolean; fsoInfo: FsoInfo }[] => {
	...
	return fsoList
		...
		.map(info => {
			...
			return { name: formatDateToYYYYMM(dt), is_default: true, fsoInfo: info };
		})
